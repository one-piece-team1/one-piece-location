 ...
 */
@Injectable()
export class LocationEventSubscribers {
  private readonly logger: Logger = new Logger('LocationEventSubscribers');
  // one server only listen to one exchange
  // seperate different event by type for different services
  private readonly defaultExchangeName: string = 'onepiece-location';

  constructor(private readonly userRepository: UserRepository) {
    this.subscribeData('onepiece_location_queue');
  }
  ...
  execute(event) {
    ...
    this.logger.log(event, 'LocationEventSubscribers');
