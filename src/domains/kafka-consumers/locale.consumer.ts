...
@Injectable()
export class LocaleKakfaConsumerService {
  private readonly logger: Logger = new Logger('LocaleKakfaConsumerService');
  private readonly consumer = new Kafka.KafkaConsumer(
    {
      'bootstrap.servers': config.EVENT_STORE_SETTINGS.bootstrapServers,
      'group.id': config.EVENT_STORE_SETTINGS.locale.groupId,
  ...
  async register(kafkaMsg: Kafka.Message) {
    ...
    console.log('LocaleKakfaConsumerService_jsonEvent: ', jsonEvent);
    if (jsonEvent.type === Event.UserEvent.CREATEUSER) {
      ...
      if (event.data) {
        ...
        if (user) {
          await this.userEventStoreRepository.register({ topic: config.EVENT_STORE_SETTINGS.topics.localeEvent }, event.id);
        }
      }
      return;
    }
  ...
  init() {
    this.consumer
      .on('ready', () => {
        this.consumer.subscribe([config.EVENT_STORE_SETTINGS.topics.localeEvent]);
        setInterval(() => {
